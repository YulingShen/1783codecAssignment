...
def a3_d1_bits(): 
    ...
    cif_1_bits = [93427, 92453, 93327, 93858, 94294, 93399, 94315,
                  87176, 86609, 86990, 86112, 87837, 86828, 86941, 92810, 91944, 93237, 92581,
                  94616, 94166, 92022] 

    cif_4_bits = [93427, 95463, 89632, 96263, 94294, 95052, 92723, 92843,
                  86609, 82937, 75264, 76982, 86828, 82021, 92157, 95310, 93237, 93804,
                  93699, 98179, 92022]

    cif_21_bits = [120320, 101303, 90601, 95118, 100504, 100687, 90315, 173815,
                  54508, 37896, 31253, 29020, 23057, 23578, 160357, 113694, 109862, 109353,
                  107281, 112088, 110693]

    qcif_1_bits = [35177, 35311, 35166, 35457, 35630, 36181, 36171, 34329,
                    34763, 34824, 34825, 34968, 33918, 34992, 35479, 35630, 36320, 36453,
                    37410, 37187, 35554]

    qcif_4_bits = [35177, 37739, 36605, 35778, 35630, 36071, 36199, 36855,
                    34763, 31612, 33610, 34588, 33918, 31790, 37298, 37602, 36320, 36327,
                    37272, 37025, 35554]

    qcif_21_bits = [40694, 32671, 31137, 27719, 32580, 32879, 27978, 54674,
                    18263, 11190, 6787, 8193, 6695, 5009, 51272, 33036, 34341, 34140, 33706,
                    34451, 38016]

    frame = 21
    mapper = {"CIF":[cif_1_bits, cif_4_bits, cif_21_bits], "QCIF":[qcif_1_bits, qcif_4_bits, qcif_21_bits]}

    plt.figure()
    for m in mapper:
        plt.plot(mapper[m][0], label="{}_I_Period=1".format(m))
        plt.plot(mapper[m][1], label="{}_I_Period=4".format(m))
        plt.plot(mapper[m][2], label="{}_I_Period=21".format(m))
    plt.title("CIF and QCIF Bit Cost")
    plt.ylabel('Bits Cost')
    plt.xlabel('Frame')
    plt.legend()
    plt.show()
    plt.close()
...
def a3_d1_psnr():
    ...
    cif_1_psnr =  [35.1, 35.13, 35.15, 35.11, 35.12, 35.1, 35.1, 39.41, 39.42,
                    39.37, 39.37, 39.4, 39.44, 39.44, 34.93, 34.9, 34.89, 34.8, 34.86, 34.85,
                    34.69]

    cif_4_psnr = [35.1, 35.7, 35.89, 36.04, 35.12, 35.58, 36.0, 31.04, 39.42,
                    40.2, 40.47, 40.69, 39.44, 40.14, 32.57, 34.14, 34.89, 35.4, 35.49,
                    35.57, 34.69]

    cif_21_psnr = [36.07, 36.22, 36.26, 36.23, 36.18, 36.14, 36.14, 35.58,
                    37.5, 38.19, 38.54, 38.78, 38.92, 39.02, 35.47, 35.89, 35.89, 35.91,
                    35.96, 35.92, 35.88]

    qcif_1_psnr = [34.5, 34.52, 34.57, 34.52, 34.48, 34.46, 34.42, 37.64,
                    37.6, 37.6, 37.62, 37.65, 37.7, 37.62, 34.44, 34.42, 34.35, 34.34, 34.35,
                    34.27, 34.09]

    qcif_4_psnr = [34.5, 35.57, 35.78, 36.01, 34.48, 35.4, 35.67, 31.49, 37.6,
                    38.76, 39.74, 40.23, 37.7, 38.79, 32.24, 34.29, 34.35, 35.22, 35.46,
                    35.06, 34.09]

    qcif_21_psnr = [35.28, 35.5, 35.47, 35.5, 35.41, 35.32, 35.33, 34.28,
                    35.96, 36.44, 36.64, 36.77, 36.9, 37.0, 34.59, 35.07, 35.1, 35.17, 35.11,
                    34.96, 34.89]

    frame = 21
    mapper = {"CIF":[cif_1_psnr, cif_4_psnr, cif_21_psnr], "QCIF":[qcif_1_psnr, qcif_4_psnr, qcif_21_psnr]}
